...
def main(args):
    ...
    plan_example_path = 'data/plan/task2plan_triplet2.json' # processed goals
    ...
    for split in ['valid_seen', 'valid_unseen']:
        sentence_save_path = os.path.join('result', 'alfred', 'roberta', '%s-sentence@%d-%d.json'%(split, args.k, int(args.k/4)))
        ...
        for ep in tqdm(os.listdir(data_path), desc=split):
            for trial in os.listdir(os.path.join(data_path, ep)):
                ...
                for i, goal in enumerate(get_goal(traj_data)):
